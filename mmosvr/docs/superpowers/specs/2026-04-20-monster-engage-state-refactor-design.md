...
행동은 **스킬 자체의 속성**이다 (같은 스킬이 몬스터가 바뀐다고 다른 방식으로 동작할 이유가 없음). 따라서 `monster_skills.csv` (N:M 조인) 가 아니라 `skill_templates.csv` / `SkillTemplate` 에 행동을 바인딩한다.

`skill_templates.csv` 스키마에 문자열 컬럼 하나 추가:

| 기존 컬럼 | 신규 |
|-----------|------|
| sid, name, targeting, projectile_*, cooldown, cost, cast_range | **`behavior`** (문자열, 비어 있거나 `"default"` 면 `DefaultAttackBehavior`) |

`MonsterSkillEntry` / `monster_skills.csv` 는 변경하지 않는다.

서버 기동 시 `SkillTable::OnLoaded` 에서:

1. 각 `SkillTemplate` 의 `behaviorName` 문자열 읽기
2. `SkillBehaviorRegistry::Create(name)` 호출 → `shared_ptr<ISkillBehavior>` 반환
3. `SkillTemplate.behavior` 에 대입 (로딩 이후 `const`)
...
3. `skill_templates.csv` 에 해당 스킬 행의 `behavior` 컬럼을 `"fireball"` 로 지정
...
- `skill_templates.csv` 의 기존 행들에 `behavior` 컬럼을 어떻게 채울지 — 전부 빈 값(`default`) 로 두는 방향으로 진행
