...
Unity 에디터 버전: **6000.4.2f1** (기존 ClaudeProject 와 동일).
...
- `Network/` — ClaudeProject 에서 포팅한 TCP/Protobuf 레이어 (NetworkManager, NetworkClient, PacketRouter, PlayerManager, NetworkPlayerSync, RemotePlayerController, MainThreadDispatcher)
- `Proto/` — protoc 생성 C# (Common, Login, Game, PacketIds)
...
동일한 흐름을 ClaudeProject 에서 그대로 사용 중:
...
`../ShareDir/generate_proto.bat` 실행 시 C# Proto 가 `../ClaudeProject/Assets/Scripts/Proto/` 로 생성됨.
AlbionClient 쪽으로도 동기화하려면 생성 스크립트 출력 경로 조정 필요 (현재는 수동 copy).
